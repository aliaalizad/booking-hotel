...
<form class="form row justify-content-center" id="reception" action="{{ route('admin.hotels.unbookables.store', $hotel->id) }}" method="post">
    @csrf
                    ...
                    <tbody>
                        ...
                        @foreach ($unbookables as $unbookable)
                            <tr class="text-center fw-bold fs-6 border-bottom border-gray-200">
                                ...
                                <td>
                                    <form id="delete-unbookable-{{$unbookable->id}}" action="{{ route('admin.hotels.unbookables.delete', ['hotel' => $hotel->id, 'unbookable' => $unbookable->id]) }}" method="post">
                                        @csrf
                                        @method('DELETE') 
                                    </form>
