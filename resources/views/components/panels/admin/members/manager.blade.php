...
<div class="card-body collapse show" id="manager">
    ...
    <div class="row mb-8">
        ...
        <!--begin::Col-->
        <div class="col-xl-8">
            <select name="manager" class="form-select" data-control="select2" data-placeholder="مدیر را انتخاب کنید" >
                    <option></option>
                @foreach ( App\Models\Manager::all() as $manager )
                    <option value="{{ $manager->id }}" @if(isset($member)) @selected($manager->id == $member->manager_id) @endif>{{ $manager->name }}</option>
                @endforeach
            </select>
        </div>
        <!--end::Col-->
