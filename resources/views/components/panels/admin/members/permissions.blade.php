    ...
    <div class="row mb-8">
        ...
        <div class="col-xl-8">
            <select name="roles[]" id="roles" class="form-select" data-control="select2" multiple="multiple" data-placeholder="نقش ها را انتخاب کنید" >
                <option></option>
                @foreach ( App\Models\Role::whereGuard("member")->get() as $role )
                    <option value="{{ $role->id }}" @if(isset($member)) @selected($member->roles->contains($role)) @endif > {{ $role->label }} </option>
                @endforeach
            </select>
        </div>
        <!--end::Col-->
        ...
        <div class="col-xl-8">
            <select name="permissions[]" id="permissions" class="form-select" data-control="select2" multiple="multiple" data-placeholder="دسترسی ها را انتخاب کنید" >
                ...
                @foreach ( App\Models\Permission::whereGuard("member")->get() as $permission )
                    <option value="{{ $permission->id }}" @if(isset($member)) @selected($member->permissions->contains($permission)) @endif > {{ $permission->label }} </option>
                @endforeach
            </select>
        </div>
        <!--end::Col-->
