...
<form>
    ...
    <div class="card mb-7">
        ...
        <div class="card-body">
            ...
            <!--end::Compact form-->
            <!--begin::Advance form-->
            <div class=@if( ! is_null(request('name')) or ! is_null(request('phone')) or ! is_null(request('status')) or ! is_null(request('province')) or  ! is_null(request('contract'))  ) {{ "" }} @else {{ 'collapse' }} @endif  id="search_advanced">
                <!--begin::Separator-->
                ...
                <div class="row g-8 mb-8">
                    ...
                    <!--begin::Col-->
                    <div class="col-xxl-4">
                        <label class="fs-6 form-label text-dark ">قرارداد</label>
                        <select name="contract" class="form-select form-select-solid" data-allow-clear="true" data-control="select2" data-placeholder="قرارداد را انتخاب کنید">
                            <option></option>
                            @foreach (App\Models\Contract::all() as $contract)
                                <option value="{{ $contract->id }}" @selected(request('contract') == $contract->id)>{{ $contract->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <!--end::Col-->
